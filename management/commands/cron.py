...
class Command(BaseCommand):
    ...
    def executeNextWaitingScan(self):
        try:
            ...
            waitingScan.filepath = nmap.genFinishedScanFileName(waitingScan.name, waitingScan.started.time().second)
            print('[CRON]  finishedFile', waitingScan.filepath)
            shutil.move(nmap_active_scan_out, waitingScan.filepath)
            print('[CVE] start')
            nmapout = cve.getcve(waitingScan.filepath)
